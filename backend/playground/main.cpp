...
static void PrintBoard(const spread_logic::Field& field) {
  ...
  for (int y = 0; y < h; ++y) {
    for (int x = 0; x < w; ++x) {
      ...
      if (cell.owner_index != 0) {
        owner = 'A' + static_cast<char>(cell.owner_index - 1);
      }
      std::cout << owner;
      std::cout << static_cast<int>(cell.fullness);
      if (x + 1 < w) {
        std::cout << ' ';
      }
    }
...
int main() {
  ...
  spread_logic::Game game(p, static_cast<std::uint8_t>(w),
                          static_cast<std::uint8_t>(h));

  while (true) {
    std::cout << "\nCurrent board (owner+fullness):\n";
    PrintBoard(game.GetField());
    if (auto alive = game.GetAlivePlayers(); alive.size() == 1) {
      std::cout << "Winner: Player "
                << static_cast<char>('A' + alive.front() - 1) << "\n";
      break;
    }
    auto active = game.GetCurrentPlayer();
    std::cout << "Player " << static_cast<char>('A' + active - 1)
              << " move (x y): ";
    int x = -1, y = -1;
    if (!(std::cin >> x >> y)) {
    ...
    auto pos = spread_logic::Coordinate{static_cast<int8_t>(x),
