...
<main class="max-w-6xl mx-auto p-4 space-y-4">
    ...
    {#if game}
    <section class="grid grid-cols-1 md:grid-cols-[1fr_280px] gap-4 items-start">
    ...
    <div class="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
        <div class="bg-slate-800 border border-slate-700 rounded-lg shadow p-6 w-80 text-center space-y-3">
            <h3 class="text-lg font-semibold">Game Over</h3>
            <p class="text-slate-300">Winner: <span class="font-mono">{game.winner}</span></p>
        </div>
    </div>
    {/if}
    {/if}
